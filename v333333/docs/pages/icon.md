...
<p class="uk-text-lead">在网站内容中任意位置使用可缩放的矢量图标。</p>

UIkit 自带 SVG 图标系统和完善的图标库，含有越来越多的线性图标。此组件将 SVG 注入到网页中，还能使用 CSS 来设置图标的颜色和其他样式。

***

## 用法

在`<span>` 或 `<a>`元素上添加 `uk-icon` 属性来使用图标组件。在该属性中添加`icon: NAME`参数来显示实际图标。然后你就有了一个可以用CSS来设置样式的矢量图标。
...
在`uk-icon`属性中添加`ratio: 2`参数，使图标尺寸翻倍，也可以基于你的需求填写其他任意数值。
...
重置默认的链接样式，淡化色彩并使用图标作为锚文本，需要用到 `.uk-icon-link` class。
...
在`<a>`元素上使用`.uk-icon-button` class来创建图标按钮。
...
还可以将任意背景图片缩小的图标的尺寸，只需要添加`.uk-icon-image` class和背景图片路径就行。
